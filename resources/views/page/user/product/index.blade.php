...
<style>
    ...
    @media (max-width: 768px) {
        .card-container {
            ...
            /* Kartu produk ditumpuk secara vertikal di layar yang lebih kecil */
        }

        .card {
            flex: 0 0 100%;
            /* Lebar penuh di layar yang lebih kecil */
        }
    }
                            ...
                            <form action="{{ route('carts.addToCart', ['productId' => $product->id]) }}" method="post">
                                ...
                                <ul class="size-list">
                                    @foreach ($product->productSizes as $size)
                                        <li>
                                            <input type="radio" name="ukuran[{{ $product->id }}]"
                                                id="{{ $size->size }}" value="{{ $size->size }}" required>
                                ...
                                <select name="quantity[{{ $product->id }}]" id="quantity">
                                    @for ($i = 1; $i <= 50; $i++)
                                        <option value="{{ $i }}" data-stok="{{ $product->stok }}">
                                            {{ $i }}</option>
                                    @endfor
                                </select>
                                <div>
                                ...
                                <a href="{{ route('detailbaju', ['id' => $product->id]) }}" class="btn-add-to-cart"
                                    style="margin-top: 4px">Detail</a>
