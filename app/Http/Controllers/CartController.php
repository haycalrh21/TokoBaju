    ...
    public function simpan(Request $request)
    {
        try {
            ...
            if ($cartId) {
                ...
                // Cari semua data Order dengan cart_id yang sesuai
                $orderan = Order::where('cart_id', $cartId)->get();

                // Periksa apakah ada entri Order yang sesuai
                if ($orderan->isNotEmpty()) {
                    // Iterasi melalui setiap entri Order dan perbarui data
                ...
                // Simpan data pengiriman setelah memperbarui data orderan
            ...
            foreach ($checkOuts as $checkOut) {
                ...
                $ongkir = Pengiriman::where('cart_id',$cartId)->first()->hargaongkir;
                // Ambil totalPrice dari semua item checkout dengan cart_id yang sama
                $totalPrice = Checkout::where('cart_id', $cartId)->first()->totalPrice;
                $jumlahharga = $totalPrice + $ongkir;
                if (!$snapToken) {
                    ...
                    $params = [
                        'transaction_details' => [
                            'order_id' => $cartId,
                            'gross_amount' => $jumlahharga, // Gunakan totalPrice dari semua item checkout
                        ],
                        'customer_details' => [
